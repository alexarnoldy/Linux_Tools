...
#### Shell script to add a new vlan and bridge, verify the deployed vlans and bridges, and remove vlans and bridges
#### Future improvements will also deploy and configure a vyos router and sles jumphost via Terraform
...
for EXISTING_BRIDGES in $(ssh ${NODE} sudo ls /etc/sysconfig/network/ifcfg-br* | grep -v br200 | awk -F-br '{print$2}')
do
...
echo Bridges to be configured on ${NODE} are: ${CREATE_NEW_BRIDGES[@]}
        ...
        [ ${RESTART_NETWORK} == 1 ] && ssh ${NODE} sudo systemctl restart network.service&
